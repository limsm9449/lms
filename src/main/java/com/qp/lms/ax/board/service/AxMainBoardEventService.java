...
@Service
public class AxMainBoardEventService {
	@Autowired
	...
	public HashMap<String, Object> axBoardEventList(HashMap<String, Object> paramMap) throws Exception {
    	...
    	hm.put("list", sqlSession.selectList("axMainBoard.axMainBoardEventList", paramMap));
	...
	public HashMap<String, Object> axBoardEventOne(HashMap<String, Object> paramMap) throws Exception {
    	...
    	hm.put("row", sqlSession.selectOne("axMainBoard.axMainBoardEventOne", paramMap));
    ...
    public HashMap<String, Object>  axBoardEventSave(HashMap<String, Object> paramMap) throws Exception {
		...
		if ( "INSERT".equals(mode) ) {
			...
			sqlSession.insert("axMainBoard.axMainBoardEventInsert", paramMap);
		} else if ( "UPDATE".equals(mode) ) {
			sqlSession.update("axMainBoard.axMainBoardEventUpdate", paramMap);
		} else if ( "VIEW".equals(mode) ) {
			sqlSession.update("axMainBoard.axMainBoardEventViewCntUpd", paramMap);
		}
		...
		if ( updList != null ) {
			for ( int i = 0; i < updList.size(); i++ ) {
				...
				sqlSession.update("axMainBoard.axMainBoardEventMainUpdate", row);
			}
		}
		...
		if ( delList != null ) {
			for ( int i = 0; i < delList.size(); i++ ) {
				sqlSession.delete("axMainBoard.axMainBoardEventDelete", delList.get(i));
				
				if ( "Y".equals(delList.get(i).get("IMG1")) ) {
					File f = new File(commSvr.getSetting("EVENT_IMG_FOLDER") + "//" + delList.get(i).get("COMP_CD") +"//" + delList.get(i).get("SEQ") + "_img1.jpg");
					f.delete();
				}
				if ( "Y".equals(delList.get(i).get("IMG2")) ) {
					File f = new File(commSvr.getSetting("EVENT_IMG_FOLDER") + "//" + delList.get(i).get("COMP_CD") +"//" + delList.get(i).get("SEQ") + "_img2.jpg");
					f.delete();
				}
			}
		}
	...
	public HashMap<String, Object> axBoardEventImageList(HashMap<String, Object> paramMap) throws Exception {
    	...
    	List<HashMap<String, Object>> list = sqlSession.selectList("axMainBoard.axMainBoardEventOne", paramMap);
    	hm.put("list", list);
    ...
    public HashMap<String, Object>  axBoardEventImageUpload(HashMap<String, Object> paramMap) throws Exception {
		...
		sqlSession.update("axMainBoard.axMainBoardEventImageUpdate", paramMap);
