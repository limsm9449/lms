package com.qp.lms.cost.model;

import java.util.List;

import com.qp.lms.common.CodeVO;
import com.qp.lms.common.CommonSet;


public class CostPaySet extends CommonSet {
	private CostPayVO condiVO;					// 조건용
	private List<CostPayVO> list;				// 리스트용
	private CostPayVO data;					// 데이타용

	private List<CostPayVO> payList;			// 과정별 지급 리스트
	private List<CostPayVO> payRegisterList;		// 지급별 수강자 리스트
	
	
	private List<CodeVO> ddCategory1Depth;		// 카테고리 대분류
	private List<CodeVO> ddCategory2Depth;		// 카테고리 중분류
	private List<CodeVO> ddCategory3Depth;		// 카테고리 소분류
	
	
	
	public CostPayVO getCondiVO() {
		return condiVO;
	}
	public void setCondiVO(CostPayVO condiVO) {
		this.condiVO = condiVO;
	}
	public List<CostPayVO> getList() {
		return list;
	}
	public void setList(List<CostPayVO> list) {
		this.list = list;
	}
	public CostPayVO getData() {
		return data;
	}
	public void setData(CostPayVO data) {
		this.data = data;
	}
	public List<CodeVO> getDdCategory1Depth() {
		return ddCategory1Depth;
	}
	public void setDdCategory1Depth(List<CodeVO> ddCategory1Depth) {
		this.ddCategory1Depth = ddCategory1Depth;
	}
	public List<CodeVO> getDdCategory2Depth() {
		return ddCategory2Depth;
	}
	public void setDdCategory2Depth(List<CodeVO> ddCategory2Depth) {
		this.ddCategory2Depth = ddCategory2Depth;
	}
	public List<CodeVO> getDdCategory3Depth() {
		return ddCategory3Depth;
	}
	public void setDdCategory3Depth(List<CodeVO> ddCategory3Depth) {
		this.ddCategory3Depth = ddCategory3Depth;
	}
	public List<CostPayVO> getPayList() {
		return payList;
	}
	public void setPayList(List<CostPayVO> payList) {
		this.payList = payList;
	}
	public List<CostPayVO> getPayRegisterList() {
		return payRegisterList;
	}
	public void setPayRegisterList(List<CostPayVO> payRegisterList) {
		this.payRegisterList = payRegisterList;
	}
	
	
	
	
	

	

}
