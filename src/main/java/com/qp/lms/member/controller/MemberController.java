...
public class MemberController {
    ...
    public String reCertificationMail(@ModelAttribute MemberVO vo, Model model) throws Exception {
    	try {
	    	...
	    	contents.append("    회원 가입을 정상적으로 처리하기 위해서는 아래 이메일 인증 주소를 클릭 하시거나 <a href='" + SessionUtil.getAttribute("serverDomain") + "/guest/certification.do?certificationKey=" + vo.getCertificationKey() + "'>여기</a>를 클릭하여 주십시오.");
	    	contents.append("    <p style='display:block;margin: 30px 10px;'><a href='" + SessionUtil.getAttribute("serverDomain") + "/guest/certification.do?certificationKey=" + vo.getCertificationKey() + "'>" + SessionUtil.getAttribute("serverDomain") + "/guest/certification.do?certificationKey=" + vo.getCertificationKey() + "</a></p>");
    		...
    		axCommService.axMailSave(set.getCondiVO().getUserId(), vo.getEmail(), "[회원가입 인증메일] 큐러닝에 가입하신 환영합니다.", contents.toString());
