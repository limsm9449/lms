...
function lfn_btn(pKind, pParam) {
	if ( pKind =="save" ) {
		...
		if ( confirm("생성하시겠습니까?") == true ) {
			...
			$.ajax({
				type :"POST",
				url : context +"/courseExam/courseExamNIns.do",
				dataType :"json",
				data : $("#frm").serialize(),
		...
		gfn_goPage('/courseExam/courseExamList'); 
	}
}
					...
					<tbody>
						<c:forEach begin="1" end="20" varStatus="idx">
				            <tr <c:if test="${idx.count%2 eq 1}"> class="alt"</c:if>>
				              	...
				              	<td class="subject"><input id="answers" name="answers" value="답" style="width:60px"/></td>
				              	<td class="subject"><input id="grades" name="grades" value="5" style="width:45px"/></td>
