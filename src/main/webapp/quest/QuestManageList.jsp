...
function lfn_btn(pKind, pParam) {
	if ( pKind == "search" || pKind == "paging" || pKind == "refresh" ) {
		...
		f_submitSelf("/quest/questManageList");	
	} else if ( pKind == "view") {
		lfn_setCondition();
		
		gfn_goPage("/quest/questUserManageList","courseId=" + pParam.courseId);
	}
}

function lfn_setCondition() {
	top.gCondition.questManage = {
					...
					<tbody>
						...
						<c:forEach var="row" items="${set.list}" varStatus="idx">
							<tr <c:if test="${idx.count%2 eq 1}"> class="alt"</c:if>>
				            	...
				            	<td class="center">${row.answer1Cnt}</td>
				            	<td class="center">${row.answer2Cnt}</td>
				            	<td class="center">${row.answer3Cnt}</td>
				            	<td class="no_line center">${row.answer4Cnt}</td>
							</tr>
						</c:forEach> 
					</tbody>
				</table>
