...
function lfn_btn(pKind, pParam) {
	if ( pKind == "search" || pKind == "paging" || pKind == "refresh" ) {
		...
		f_submitSelf(preUrl + "approvalCompanyList");	
	} else if ( pKind == "refund" ) {
		gfn_popup(preUrl + "approvalCompanyRefundP","courseId="+pParam.courseId,400,300);
	}
}
			...
			<tbody>
				<c:forEach var="row" items="${set.list}" varStatus="idx">
		            <tr <c:if test="${idx.count%2 eq 1}"> class="alt"</c:if>>
		            	...
		            	<td>${row.compName}</td>
		            	<td><a href="#" onclick="javascript:Popup.showCompanyRegister('${row.courseId}'); return false;">${row.empCnt}</a></td>
		            	<td><fmt:formatNumber value="${row.paymentCost}" type="number"/></td>
		            	<td>${row.paymentDate}</td>
		            	<td><a href="#" onclick="javascript:lfn_btn('refund',{courseId:'${row.courseId}'}); return false;">환불</a></td>
		            </tr>
				</c:forEach>
			</tbody>
		</table>
