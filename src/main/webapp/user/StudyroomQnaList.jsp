...
function lfn_btn(pKind, pParam) {
	if ( pKind == "search" || pKind == "paging" || pKind == "refresh" ) {
		...
		f_submitSelf("/user/studyroomQnaList");
	} else if ( pKind == "new") {
		gfn_goPage("/user/studyroomQnaN",$("#frm").serialize());
	} else if ( pKind == "view") {
		$("#seq").val(pParam.seq);
		            ...
		            <div id="tab3" class="tab_content" style="display:block;">
		              ...
		              <table class="bod_list">
		                <caption>QnA 목록</caption>
							...
							<c:forEach var="row" items="${set.boardQnaList}" varStatus="idx">
				              <tr <c:if test="${idx.index + 1 eq fn:length(set.boardQnaList)}"> class="last_line"</c:if>>
		              ...
		              <%-- 페이징 --%>
	        			...
	        			<div class="right_btn up_btn">
	        			  <a href="#" onClick="javascript:lfn_btn('new'); return false;"><img src="/resources/images/sub/btn_qna.png" alt="질문하기" /></a>
	        			</div>
		            </div>
		            <!-- end tab_3 -->
