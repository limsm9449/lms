...
function lfn_btn(pKind, pParam) {
	if ( pKind == "search" || pKind == "paging" || pKind == "refresh" ) {
		...
		f_submitSelf(preUrl + "costApprovalList");	
	} else if ( pKind == "refundV" ) {
		lfn_setCondition();
		
		gfn_goPage(preUrl + "costApprovalRefundV","approvalId="+pParam.approvalId);
	}
}
...
<c:choose>
	<c:when test="${row.costIdCnt eq '0'}">
				            	<td class="no_line center"><a href="#" onclick="javascript:lfn_btn('refundV',{approvalId:'${row.approvalId}'}); return false;" class="click">환불</a></td>
	</c:when>
	<c:otherwise>
				            	<td class="no_line center">환불</td>
	</c:otherwise>
</c:choose>		
							</tr>
						</c:forEach> 
