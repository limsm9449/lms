...
function lfn_btn(pKind, pParam) {
	if ( pKind == "search" || pKind == "paging" || pKind == "refresh" ) {
		...
		f_submitSelf("/cost/costNotPayList");	
	} else if ( pKind == "update") {
		lfn_setCondition();
		
		gfn_goPage("/cost/costNotPayU","courseId=" + pParam.courseId);
	}
	
}

function lfn_setCondition() {
	top.gCondition.costNotPay = {
					...
					<tbody>
						...
						<c:forEach var="row" items="${set.list}" varStatus="idx">
							<tr <c:if test="${idx.count%2 eq 1}"> class="alt"</c:if>>
				              	...
				              	<td class="right"><a href="#" onclick="javascript:lfn_btn('update',{courseId:'${row.courseId}'}); return false;" class="click"><fmt:formatNumber value="${row.notPaymentCost}" type="number"/></a></td>
				              	<td class="right">${row.registerCnt}</td>
				              	<td class="no_line right">${row.costNotPayCnt}</td>
							</tr>
						</c:forEach> 
					</tbody>
				</table>
