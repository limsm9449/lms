...
function lfn_btn(pKind, pParam) {
	if ( pKind == "search" || pKind == "paging" || pKind == "refresh" ) {
		...
		f_submitSelf("/exam/examManageList");	
	} else if ( pKind == "view") {
		lfn_setCondition();
		
		gfn_goPage("/exam/examUserManageList","courseId=" + pParam.courseId);
	}
}

function lfn_setCondition() {
	top.gCondition.examManage = {
					...
					<tbody>
						...
						<c:forEach var="row" items="${set.list}" varStatus="idx">
							<tr <c:if test="${idx.count%2 eq 1}"> class="alt"</c:if>>
				            	...
				            	<td class="center">${row.over90}</td>
				            	<td class="center">${row.over80}</td>
				            	<td class="center">${row.over70}</td>
				            	<td class="no_line center">${row.under70}</td>
							</tr>
						</c:forEach> 
					</tbody>
				</table>
