...
function lfn_btn(pKind, pParam) {
	if ( pKind == "update" ) {
		f_submitSelf("/companyCourse/companyCourseU",$("#courseId").val());
	} else if ( pKind =="delete" ) {
		if ( confirm("삭제하시겠습니까?") == true ) {
			btnUnbind("delBtn");
			$.ajax({
				type :"POST",
				url : context +"/companyCourse/companyCourseDel.do",
				dataType :"json",
				data : $("#frm").serialize(),
    	...
    	<div id="contents">
			...
			<div id="article">
				...
				<div class="course_tool">
					<a href="#" onclick="javascript:Popup.showViewCourse('${set.data.courseId}','1','${set.data.hPx + 100}','${set.data.vPx + 100}'); return false;">과정보기</a>
				...
				<table>
					<thead>
						...
						<tr>
							...
							<th>핸드폰</th>
							<th>성별</th>
							<th class="no_line">생년월일</th>
						</tr>
					</thead>
					<tbody>
						<c:if test="${empty set.registerList}">
							<tr>
								<td colspan="6" class="no_line center">수강생이 없습니다.</td>
							</tr>
						</c:if>
						<c:forEach var="row" items="${set.registerList}" varStatus="idx">
				            <tr <c:if test="${idx.count%2 eq 1}"> class="alt"</c:if>>
				            	...
				            	<td class="subject center">${row.sexName}</td>
				            	<td class="no_line center">${row.birthDay}</td>
				            </tr>
						</c:forEach>
					</tbody>
				</table>
