...
function lfn_btn(pKind, pParam) {
	if ( pKind == "search" || pKind == "paging" || pKind == "refresh" ) {
		...
		f_submitSelf("/quest/questGroupList");	
	} else if ( pKind == "new" ) {
		lfn_setCondition();
		
		gfn_goPage("/quest/questGroupN");
	} else if ( pKind == "view") {
		lfn_setCondition();
		
		gfn_goPage("/quest/questGroupV","qgId=" + pParam.qgId);
	}
}

function lfn_setCondition() {
	top.gCondition.questGroup = {
	...
	<div id="container">
    	<div id="contents">
			...
			<div id="article">
		   		...
		   		<table summary="">
					...
					<tbody>
						<c:if test="${empty set.list}">
							<tr>
								<td colspan="3" class="no_line center"><spring:message code="lms.msg.noSearchData" text="-" /></td>
							</tr>
						</c:if>
				...
				<%-- 하단버튼 --%>
				<div class="tool_btn">
					<a href="#" onclick="javascript:lfn_btn('new'); return false;" class="blueBtn">설문지 그룹 생성</a>
				</div>
		   	</div>
		</div>
	</div>
