...
function f_preSubmit(pDoUrl,pParam) {
	if ( pDoUrl == '/board/insBoardNotice' ) {
		if ($("#title").val() == "") {
		    alert('제목을 입력해 주세요.');
		    $("#title").select();
		    return;
...
function lfn_btn(pKind, pParam) {
	if ( pKind =="save" ) {
		...
		$.ajax({
			type :"POST",
			url : context + preUrl + "boardNoticeIns.do",
			dataType :"json",
			data : $("#frm").serialize(),
			...
			<div id="article">
				...
				<table class="input">
					...
					<tbody>
						<tr>
							<th>제목</th>
							<td class="no_line subject"><input id="title" name="title" type="text" value=""/></td>
						</tr>
						<tr>
							<th>내용</th>
							<td class="textbox subject no_line">
