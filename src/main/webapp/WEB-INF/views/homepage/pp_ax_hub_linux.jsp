...
<%@ page import="com.qp.lms.main.model.MainSet"%>
<%@ page import="com.qp.lms.common.PgService"%>
<%
    /* ============================================================================== */
...
<%
	MainSet mainSet = (MainSet)request.getAttribute("set");
...
System.out.println("req_tx : " + req_tx);
    ...
    if ( tran_cd.length() > 0 )
    {
    	...
    	System.out.println("m_res_cd : " + c_PayPlus.m_res_cd);
    	System.out.println("m_res_msg : " + c_PayPlus.m_res_msg);
    ...
    /* = -------------------------------------------------------------------------- = */
        if ( res_cd.equals( "0000" ) )
        {
            ...
            try {
            	...
            	svr.approval(request);
            	bSucc = "true";
            } catch ( Exception e ) {
            	bSucc = "false";
                System.out.println("PgService Error");
            }
		}
...
<html xmlns="http://www.w3.org/1999/xhtml" >
    <head>
        ...
        <script type="text/javascript">
            function goResult()
            {
                var openwin = window.open( '/homepage/proc_win.html', 'proc_win', '' )
    ...
    <form name="pay_info" method="post" action="/main/result.do">
        ...
        <input id="courseId" name="courseId" type="hidden" value="<%=mainSet.getCondiVO().getCourseId()%>"/>
		<input id="totalCost" name="totalCost" type="hidden" value="<%=mainSet.getCondiVO().getTotalCost()%>"/>
		<input id="paymentPoint" name="paymentPoint" type="hidden" value="<%=mainSet.getCondiVO().getPaymentPoint()%>"/>
		<input id="paymentCost" name="paymentCost" type="hidden" value="<%=mainSet.getCondiVO().getPaymentCost()%>"/>
		<input id="paymentKind" name="paymentKind" type="hidden" value="<%=mainSet.getCondiVO().getPaymentKind()%>"/>
		<input id="paymentBank" name="paymentBank" type="hidden" value="<%=mainSet.getCondiVO().getPaymentBank()%>"/>
		<input id="approvalId" name="approvalId" type="hidden" value="<%=mainSet.getCondiVO().getApprovalId()%>"/>
