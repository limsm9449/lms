...
function lfn_btn(pKind, pParam) {
	if ( pKind == "search" || pKind == "paging" || pKind == "refresh" ) {
		...
		f_submitSelf("/courseMaster/courseMasterList");	
	} else if ( pKind == "new" ) {
		lfn_setCondition();
		
		gfn_goPage("/courseMaster/courseMasterN");
	} else if ( pKind == "view") {
		lfn_setCondition();
		
		gfn_goPage("/courseMaster/courseMasterV","courseCode=" + pParam.courseCode);
	}

}

function lfn_setCondition() {
	top.gCondition.courseMaster = {
...
<div id="wrap">
	<div id="container">
    	<div id="contents">
			...
			<div id="article">
		    	<h3 class="title">과정 Master 관리</h3>
					...
					<tbody>
						...
						<c:forEach var="row" items="${set.list}" varStatus="idx">
							<tr <c:if test="${idx.count%2 eq 1}"> class="alt"</c:if>>
				            	...
				            	<td class="center">${row.tutorName}</td>
				            	<td class="center">${row.subCnt}</td>
				            	<td class="right">${row.weekCostYnName}</td>
				            	<td class="right">${row.reportRate}</td>
				            	<td class="right">${row.examRate}</td>
				            	<td class="right">${row.discussionRate}</td>
				            	<td class="no_line right">${row.progressRate}</td>
							</tr>
						</c:forEach>
					</tbody>
				</table>
