...
function lfn_btn(pKind, pParam) {
	if ( pKind == "search" || pKind == "paging" || pKind == "refresh" ) {
		...
		f_submitSelf("/courseCode/courseCodeList");	
	} else if ( pKind == "new" ) {
		lfn_setCondition();
		
		gfn_goPage("/courseCode/courseCodeN");
	} else if ( pKind == "view") {
		lfn_setCondition();
		
		gfn_goPage("/courseCode/courseCodeV","courseCode=" + pParam.courseCode);
	}
	
}

function lfn_setCondition() {
	top.gCondition.courseCode = {
    	...
    	<div id="contents">
			...
			<div id="article">
				...
				<%-- 조건 --%>
		   		...
		   		<div style="text-align:right;margin-right:10px;">${set.totalCount} 건</div>
				<%-- 테이블 --%>
		   		<table summary="">
					<thead>
					  	<tr class="guide">
					...
					<tbody>
						...
						<c:forEach var="row" items="${set.list}" varStatus="idx">
							<tr <c:if test="${idx.count%2 eq 1}"> class="alt"</c:if>>
								...
								<td class="subject">${row.hPx} * ${row.vPx}</td>
								<td class="subject">${row.directory}</td>
								<td class="right">${row.point}</td>
								<td class="center">${row.questGroupYn}</td>
				...
				<%-- 하단버튼 --%>
				<div class="tool_btn">
					<a href="#" onclick="javascript:lfn_btn('new'); return false;" class="blueBtn">과정 코드 생성</a>
				</div>
		   	</div>
		</div>
	</div>
