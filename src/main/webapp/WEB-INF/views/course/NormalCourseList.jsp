...
function lfn_btn(pKind, pParam) {
	if ( pKind == "search" || pKind == "paging" || pKind == "refresh" ) {
		...
		f_submitSelf("/normalCourse/normalCourseList");	
	} else if ( pKind == "new" ) {
		lfn_setCondition();
		
		gfn_goPage("/normalCourse/normalCourseN");
	} else if ( pKind == "view") {
		lfn_setCondition();
		
		gfn_goPage("/normalCourse/normalCourseV","courseId=" + pParam.courseId);
	}
}
	...
	<div id="container">
    	<div id="contents">
			...
			<div id="article">
				...
				<%-- 하단버튼 --%>
				<div class="tool_btn">
					<a href="#" onclick="javascript:lfn_btn('new'); return false;" class="blueBtn">일반강좌 생성</a>
				</div>
		   	</div>
		</div>
	</div>
