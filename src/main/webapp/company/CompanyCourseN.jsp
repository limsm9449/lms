...
function lfn_btn(pKind, pParam) {
	if ( pKind == "refresh" ) {
		f_submitSelf("/companyCourse/companyCourseN",$("#frm").serialize());
	} else if ( pKind =="save" ) {
		...
		if ( confirm("생성하시겠습니까?") == true ) {
			...
			$.ajax({
				type :"POST",
				url : context +"/companyCourse/companyCourseNIns.do",
				dataType :"json",
				data : $("#frm").serialize(),
				...
				<table class="input">
					...
					<tbody>
						...
						<tr>
							...
							<td class="no_line">
								<select id="compCd" name="compCd">
									<c:forEach var="row" items="${set.ddCompany}">
