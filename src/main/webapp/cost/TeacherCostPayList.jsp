...
function lfn_btn(pKind, pParam) {
	if ( pKind == "search" || pKind == "paging" || pKind == "refresh" ) {
		...
		f_submitSelf("/cost/teacherCostPayList");	
	} else if ( pKind == "view") {
		lfn_setCondition();
		
		gfn_goPage("/cost/teacherCostPayV","courseId=" + pParam.courseId);
	}
					...
					<tbody>
						...
						<c:forEach var="row" items="${set.list}" varStatus="idx">
				            <tr <c:if test="${idx.count%2 eq 1}"> class="alt"</c:if>>
				              	...
				              	<td class="no_line right"><a href="#" onclick="javascript:lfn_btn('view',{courseId:'${row.courseId}'}); return false;" class="click"><fmt:formatNumber value="${row.teacherCost}" type="number"/></a></td>
				            </tr>
						</c:forEach>
					</tbody>
				</table>
