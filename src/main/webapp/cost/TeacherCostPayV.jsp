...
function lfn_btn(pKind, pParam) {
	if ( pKind =="refresh" ) {
		gfn_goPage("/cost/teacherCostPayV","courseId=${set.data.courseId}&costId=" + pParam.costId);
	} else if ( pKind =="list" ) {
		gfn_goPage('/cost/teacherCostPayList',top.gCondition.tutorCostPay.param); 
	}
}
					...
					<tbody>	
						<c:forEach var="row" items="${set.payList}" varStatus="idx">
				            <tr <c:if test="${idx.count%2 eq 1}"> class="alt"</c:if>>
				              	...
				              	<td class="center">${row.teacherBank}</td>
				              	<td class="no_line center">${row.teacherAccNum}</td>
				            </tr>
						</c:forEach>
					</tbody>
				</table>
