...
$(document.body).ready(function () {
    ...
    $( window ).resize( function() {
    	if ( MODE == "" ) {
			gfn_gridResize("grid-parent", grid, 420);
    	}
	} ).resize();
...
function fn_search() {
	...
	gfn_callAjax("/board/axBoardFreeOne.do", params, fn_callbackAjax, "search");
}
...
function fn_callbackAjax(data, id) {
	...
	if ( id == "search" ) {
		...
		if ( data.RtnMode && data.RtnMode != "OK" ) {
			...
			mask.open();
			dialog.alert( { msg : "저장 되었습니다." }, function () { mask.close(); $("#REPLY_CONTENTS").val(""); } );
		}
	}
}
   	...
   	confirmDialog.confirm(
       	...
       	function(){
         	if ( this.key == "ok" ) {
         		...
         		gfn_callAjax("/board/axBoardFreeSave.do", saveParams, fn_callbackAjax, "save");
           	} else {
           		mask.close();
           	}
       	}
       	...
       	function(){
         	if ( this.key == "ok" ) {
         		if ( kind == "INSERT" ) {
        			...
        			var row = grid.getList("selected");
        			grid.setValue(row[0].__index, "CONTENTS", $("#REPLY_CONTENTS").val());
         		}
         		
         		gfn_callAjax("/board/axBoardFreeReplySave.do", gfn_getSaveData(grid), fn_callbackAjax, "replySave");
           	} else {
           		mask.close();
           	}
       	}
...
function fn_gridEvent(event, obj) {
	if ( event == "Click" ) {
		...
		$("#REPLY_CONTENTS").val(obj.item.CONTENTS);
...
<form class="form-horizontal">
  	...
  	<div class="form-group" id="gridDiv">
    	<label for="inputPassword3" class="col-sm-2 control-label">댓글</label>
    	<div class="col-sm-10">
