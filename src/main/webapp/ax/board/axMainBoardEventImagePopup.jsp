...
function fn_search() {
	...
	gfn_callAjax("/board/axMainBoardEventImageList.do", params, fn_callbackAjax, "search");
}

function fn_callbackAjax(data, id) {
	if ( id == "search" ) {
		if ( data.list[0].IMG1 == "Y" ) {
			$('#img1').attr("src", "/cImage/event/" + params.COMP_CD + "/" + params.SEQ + "_img1.jpg" + "?timestamp=" + gfn_timestamp());
		}
		if ( data.list[0].IMG2 == "Y" ) {
			$('#img2').attr("src", "/cImage/event/" + params.COMP_CD + "/" + params.SEQ + "_img2.jpg" + "?timestamp=" + gfn_timestamp());
		}
	} else if ( id == "save" ){
		mask.close();
...
function fn_upload(pParam) {
	...
	document.frm.action = context + "/board/axMainBoardEventImageUpload.do";
