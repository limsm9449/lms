...
function lfn_btn(pKind, pParam) {
	if ( pKind == "search" || pKind == "paging" || pKind == "refresh" ) {
		...
		f_submitSelf(preUrl + "courseList");	
	} else if ( pKind == "evalList") {
		lfn_setCondition();
		
		gfn_goPage(preUrl + "courseEvalList","courseId=" + pParam.courseId);
	}
}

function lfn_setCondition() {
	top.gCondition.evaluationCourse = {
					...
					<tbody>
						...
						<c:forEach var="row" items="${set.list}" varStatus="idx">
							<tr <c:if test="${idx.count%2 eq 1}"> class="alt"</c:if>>
				            	...
				            	<td class="subject"><a href="#" onclick="javascript:Popup.showCourseMaster('${row.courseCode}'); return false;" class="click">${row.courseName}</a></td>
				            	<td class="right"><a href="#" onclick="javascript:Popup.showCourse('${row.courseId}','${row.chasu}'); return false;" class="click">${row.chasu}</a></td>
				            	<td class="right">${row.reportCnt}</td>
				            	<td class="right">${row.examCnt}</td>
				            	<td class="right">${row.discussionCnt}</td>
				            	<td class="right">${row.progressCnt}</td>
		            			<td class="no_line right"><a href="#" onclick="javascript:lfn_btn('evalList',{courseId:'${row.courseId}'}); return false;" class="click">${row.totalCnt}</a></td>
							</tr>
						</c:forEach> 
					</tbody>
				</table>
