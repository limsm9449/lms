...
function lfn_btn(pKind, pParam) {
	if ( pKind == "search" || pKind == "paging" || pKind == "refresh" ) {
		...
		f_submitSelf(preUrl + "userBoardDiscussionList");
	} else if ( pKind == "new" ) {
		lfn_setCondition();
		...
		gfn_goPage(preUrl + "userBoardDiscussionV",$("#frm").serialize());
	} else if ( pKind == "close") {
		self.close();
	}
}
...
function lfn_setCondition() {
	opener.top.gCondition.boardDiscussion = {
		...
		<tbody>
			...
			<c:forEach var="row" items="${set.list}" varStatus="idx">
				<tr <c:if test="${idx.index + 1 eq fn:length(set.list)}"> class="last_line"</c:if>>
	              	...
	              	<td class="title2">
	              		<c:if test="${row.step > 1}">
		              		<c:forEach var="f" varStatus="i" begin="2" end="${row.step}">&nbsp;</c:forEach>
		              		<img src="<c:url value="/resources/images/icon/ico_discussion_reply.gif"/>">	
					 	</c:if>
	              		<span><a href="#" onClick="javascript:lfn_btn('view',{seq:'${row.seq}'}); return false;" class="click"><c:out value="${row.title}" escapeXml="true" /></a></span>
	              	</td>
