...
function lfn_btn(pKind, pParam) {
	if ( pKind == "search" || pKind == "paging" || pKind == "refresh" ) {
		...
		f_submitSelf(preUrl + "boardDiscussionList");
	} else if ( pKind == "new" ) {
		lfn_setCondition();
		...
		gfn_goPage(preUrl + "boardDiscussionV",$("#frm").serialize());
	} else if ( pKind == "close") {
		self.close();
	}
}
...
function lfn_setCondition() {
	top.gCondition.boardDiscussion = {
						...
						<c:forEach var="row" items="${set.list}" varStatus="idx">
				            <tr <c:if test="${idx.count%2 eq 1}"> class="alt"</c:if>>
				              	...
				              	<td class="subject">
				              		...
				              		<span><a href="#" onClick="javascript:lfn_btn('view',{seq:'${row.seq}'}); return false;" class="click">${row.title}</a></span>
				              	</td>
