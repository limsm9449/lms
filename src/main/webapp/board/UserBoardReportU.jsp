...
$(document).ready(function() {
	...
	$("#attachFrame").attr("src","/board/attachU.do?" + "pSeq=${set.condiVO.seq}&kind=B_REPORT");
});
...
function lfn_btn(pKind, pParam) {
	if ( pKind =="save" ) {
		...
		$.ajax({
			type :"POST",
			url : context + preUrl + "boardReportUpd.do",
			dataType :"json",
			data : $("#frm").serialize(),
    ...
    <table class="bod_input">
      <caption>input</caption>
        ...
        <tr>
          ...
          <td><input id="title" name="title" type="text"  value="<c:out value="${set.data.title}" escapeXml="true" />" style="width:100%"/></td>
        </tr>
        <tr class="last_line">
