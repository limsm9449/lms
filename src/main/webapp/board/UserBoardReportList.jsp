...
function lfn_btn(pKind, pParam) {
	if ( pKind == "search" || pKind == "paging" || pKind == "refresh" ) {
		...
		f_submitSelf(preUrl + "userBoardReportList");
	} else if ( pKind == "new" ) {
		lfn_setCondition();
		
		gfn_goPage(preUrl + "userBoardReportN",$("#frm").serialize());
	} else if ( pKind == "view") {
		lfn_setCondition();
		
		$("#seq").val(pParam.seq);
		
		gfn_goPage(preUrl + "userBoardReportV",$("#frm").serialize());
	} else if ( pKind == "close") {
		self.close();
	}
}
...
function lfn_setCondition() {
	opener.top.gCondition.boardReport = {
