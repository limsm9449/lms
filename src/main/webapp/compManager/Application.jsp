...
function lfn_btn(pKind, pParam) {
	if ( pKind == "approval") {
		...
		if ( $("#paymentKind").val() == "CASH" ) {
			var today = new Date();
			$("#approvalId").val(today.getTime());

			$.ajax({
				type :"POST",
				url : context + preUrl + "approval.do",
				dataType :"json",
				data : $("#frm").serialize(),
			...
			gfn_popup(preUrl + "approvalP","courseId=" + $("#courseId").val() + 
...
function lfn_validate() {
	if ( $("#paymentKind").val() == "CASH" && $("#paymentBank").val() == "" ) {
...
function lfn_pay() {
	...
	if ( $("#pay_method").val() == "CASH" ) {
		...
		$.ajax({
			type :"POST",
			url : context +"/compCourseReg/approval.do",
			dataType :"json",
			data : $("#order_info").serialize(),
			success : function(json){
				if ( json.rtnMode == "OK") {
					alert("입금 확인후에 수강신청이 완료됩니다.\n승인후에 나의 강의실에서 과정을 들으시면 됩니다.");
					page.goPage("/compCourseReg/compCourseRegList");
				}
			},
			error : function(e) {
				alert("<spring:message code="lms.msg.systemError" text="-" />");
...
<form id="order_info" name="order_info" method="post" action="/compCourseReg/ppAxHub.do" onsubmit="return false;">
	...
	<input id="selIds" name="selIds" type="hidden" value="${set.userIds}"/>
		      ...
		      <div class="artcle">
		        <div class="payment_box">
		          ...
		          <table class="cart_list">
		          	<caption>결제 리스트</caption>
			              ...
			              <tr>
			              	<td class="center"></td>
			                <td class="title">
								인원 : ${set.applicationCnt} 명<br>
								<c:forEach var="row" items="${set.applicationUserList}" varStatus="idx">
									${row.userName},
								</c:forEach>
			               	</td>
			                <td class="center"><fmt:formatNumber value="${set.paymentCost}" type="number"/> 원</td>
			              </tr>
		            </tbody>
		          </table>
		          <div id="payment_total">
		          	<p>
			          	주문금액 :
			          	<span class="blue big"><fmt:formatNumber value="${set.paymentCost}" type="number"/></span>
			          	원
			          	...
			          	<span class="blue big" id="paymentCost1"><fmt:formatNumber value="${set.paymentCost}" type="number"/></span>
			          	원
			          	<input id="totalCost" name="totalCost" type="hidden" readonly value="${set.paymentCost}"/>
			          	<input id="remainPoint" name="remainPoint" type="hidden" value="${set.point}"/>
			          	<input id="paymentCost" name="paymentCost" type="hidden" value="${set.paymentCost}"/>
			          	<input type="hidden" name="good_mny" id="good_mny" class="w100" readonly value="${set.paymentCost}" maxlength="9"/>
		          	</p>
		          </div>
		          ...
		          <table class="mem_form">
		            <caption>결제 선택</caption>
		              ...
		              <tr>
		                ...
		                <td><p class="total">최종결제 금액 : <span class="blue big" id="paymentCost2"><fmt:formatNumber value="${set.paymentCost}" type="number"/></span>원</p></td>
		              </tr>
		            </tbody>
		          </table>
